...
## Read in IPD
targetpop <- read_csv("Data/fake_ipd_novarnames.csv", guess_max = 1E8)

# Get list of continuous and categorical variables
...
charstatus <- map2_chr(names(targetpop), targetpop, ~ {
  ...
  if(GetCatVar(.y)) "categorical" else "numerical"
})
contvar <- names(targetpop)[charstatus == "numerical"]
catvar <- names(targetpop)[charstatus == "categorical"]
